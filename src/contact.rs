...
impl Contact {
    pub async fn load_from_db(context: &Context, contact_id: i64) -> crate::sql::Result<Self> {
        ...
        if contact_id == DC_CONTACT_ID_SELF {
            contact.name = context.stock_str(StockMessage::SelfMsg).await.to_string();
    ...
    pub(crate) async fn add_or_lookup(
        ...
        if let Ok((id, row_name, row_addr, row_origin, row_authname)) = context.sql.fetch_one(
            ...
            if context
                .sql
                .execute(
                    sqlx::query("INSERT INTO contacts (name, addr, origin, authname) VALUES(?, ?, ?, ?);")
                        .bind(
                        name.as_ref().to_string()).bind(
                        &addr).bind(
                        origin).bind(
    ...
    pub async fn get_all_blocked(context: &Context) -> Vec<i64> {
        context
            .sql
            .query_map(
