...
impl ChatId {
    ...
    pub async fn get_for_contact(context: &Context, contact_id: u32) -> Result<Self> {
        ChatIdBlocked::get_for_contact_id(context, contact_id, Blocked::Not)
            .await
            .map(|chat| chat.id)
    }
...
impl ChatIdBlocked {
    ...
    async fn lookup_by_contact_id(
        ...
        context
            ...
            .query_row_optional(
                ...
                |row| {
                    Ok(ChatIdBlocked {
                        id: row.get::<_, ChatId>(0)?,
                        blocked: row.get::<_, Blocked>(1)?,
                    })
                },
            )
            .await
    ...
    /// state.
    pub async fn get_for_contact_id(
        context: &Context,
        contact_id: u32,
        ...
        if let Some(res) = Self::lookup_by_contact_id(context, contact_id).await? {
        ...
        Self::lookup_by_contact_id(context, contact_id)
            .await
            .and_then(|maybe| {
                maybe.ok_or_else(|| Error::msg("Unknown failure creating ChatId for contact"))
            })
    }
}
...
mod tests {
    ...
    async fn test_lookup_by_contact_id() {
        ...
        let chat_id = ChatIdBlocked::get_for_contact_id(&ctx, contact_id, Blocked::Manually)
            .await
            .unwrap()
            .id;
        let (chat_id2, blocked) = lookup_by_contact_id(&ctx, contact_id).await.unwrap();
